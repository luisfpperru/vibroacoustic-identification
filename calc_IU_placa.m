function calc_IU_placa(L,modos,Ne,frequencia)
    %%%%%%%%%%%%% Designando variï¿½veis de entrada %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    close all;
    L = L(:);
    modos = modos(:);
    Ne = Ne(:);
    
    Lx = L(1);      % Comprimento da placa na direï¿½ï¿½o x
    Ly = L(2);      % Comprimento da placa na direï¿½ï¿½o y
%     Lz = L(3);
    m = modos(1);   % modo de vibraï¿½ï¿½o na direï¿½ï¿½o x
    n = modos(2);   % modo de vibraï¿½ï¿½o na direï¿½ï¿½o y
    Ne_x = Ne(1);  % nï¿½mero de elementos na direï¿½ï¿½o x
    Ne_y = Ne(2);  % nï¿½mero de elementos na direï¿½ï¿½o y
    
    %%%%%%%%%%%%% Constantes  Fï¿½sicas  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    c = 343;  % velocidade do som
    rho = 1.29; % massa especï¿½ficado meio (ar)
        
    %%%%%%% Calculo de variï¿½veis importantes na placa %%%%%%%%
...
%     tic % inicia o cronometro
    ...
    A = Lx*Ly;  % ï¿½rea superficial da placa
    Ne = Ne_x*Ne_y; % nï¿½mero total de elementos na malha
    Np = (Ne_x + 1)*(Ne_y + 1);
   
    kx = m*pi/Lx; % nï¿½mero de onda na direï¿½ï¿½o x
    ky = n*pi/Ly; % nï¿½mero de onda na direï¿½ï¿½o y
    kf = sqrt(kx^2+ky^2); % nï¿½mero de onda livre
%     frequencia = ipsilon*kf*c/(2*pi); % frequencia de excitaï¿½ï¿½o
    omega = 2*pi*frequencia; %  frequencia angular
    k = omega/c; % nï¿½mero de onda
    ipsilon = k/kf; % frequencia adimensional
    fprintf('A frequência adimensional será: %f \n',ipsilon)
    delta = 10^(-7); % tolerancia do truncamento

    
    %%%%%%%%%%%%% Determinaï¿½ï¿½o do modo de vibraï¿½ï¿½o %%%%%%%%%%%%%%
    ...
    for i = 1:Ne_x + 1
        for j =1:Ne_y + 1
            D(i,j) = 2/sqrt(A)*sin(kx*x(i))*sin(ky*y(j))*10^(-6);   % Calculo do deslocamento na superfï¿½cie da placa pela formula analï¿½tica
        end
    end 
    ...
    kx1 = ifftshift( (2*pi/Lx)*(-Ne_x/2:Ne_x/2)  ); % nï¿½meros de onda da direï¿½ï¿½o x 
    ky1 = ifftshift( (2*pi/Ly)*(-Ne_y/2:Ne_y/2)  ); % nï¿½meros de onda da direï¿½ï¿½o y
...
%     
    ...
    P = ifft2(tP);   % pressao 
    I2 = 1/2*real(P.*V'.'); % intensidade acï¿½stica
    Pot2 = integral_dupla(Lx/Ne_x, Ly/Ne_y, I2); % Potï¿½ncia sonora
    NPS2 = 10*log10((Pot2)/(10^(-12)));
    fprintf('Intensidade Acústica Convencional - NPS: %f \n',NPS2)
    
    %%%%%%%%%%%%%%%%% Calculo das Grandezas Supersï¿½nicas %%%%%%%%%%%%%%%%
     
    clear D V P kz
    tPS = zeros(Ne_x + 1,Ne_y + 1);  % pressï¿½o angular supersï¿½nica
    tVS = zeros(Ne_x + 1,Ne_y + 1);  % velocidade angular supersï¿½nica
    for i = 1:Ne_x + 1
        for j =1:Ne_y + 1
    ...
    IS2 = 1/2*real(PS.*conj(VS)); % intensidade acï¿½stica
    Pot2 = integral_dupla(Lx/Ne_x, Ly/Ne_y, IS2); % Potï¿½ncia sonora
    NPS_IS2 = 10*log10((Pot2)/(10^(-12)));
    fprintf('Intensidade Supersônica - NPS: %f \n',NPS_IS2)
...
%     toc % para o cronï¿½metro e exibe o tempo de operaï¿½ï¿½o
    ...
    subplot(3,3,1)
    plotagrandeza(coord,conec,imag(v))% plota a intensidade acústica
...
%     3D figures
    ...
    figure
    plotagrandeza2(x,y,imag(reshape(v,Ne_x+1,Ne_y+1)))% plota a intensidade acústica
