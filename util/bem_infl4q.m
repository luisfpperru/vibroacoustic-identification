function [He,Ge]=bem_infl4q(coord,ex,ey,ez,ep,n)
...
k=ep(1)/ep(2);
%****Gauss points****
ga=0.577350269189626; x=coord(1); y=coord(2); z=coord(3);
...
%****Influence Vectors****
xdis=xg-x; ydis=yg-y; zdis=zg-z;
dis=sqrt(xdis.^2+ydis.^2+zdis.^2);
g=i*ep(3)*ep(1)*exp(-i*k*dis)./(4*pi*dis);
Ge(1,1)=sum(g.*N(:,1).*A); 
Ge(1,2)=sum(g.*N(:,2).*A);
Ge(1,3)=sum(g.*N(:,3).*A); 
Ge(1,4)=sum(g.*N(:,4).*A);
h1=-xdis.*exp(-i*k*dis)./(4*pi*dis.^2).*(i*k+1./dis);
h2=-ydis.*exp(-i*k*dis)./(4*pi*dis.^2).*(i*k+1./dis);
h3=-zdis.*exp(-i*k*dis)./(4*pi*dis.^2).*(i*k+1./dis);
